...
class LabTreeBuilderTest {
    ...
    fun `a Compact container computes no preview or summary`() {
        ...
        val root = nodes.single { it.depth == 1 } as LabNode.Container
        val nested = nodes.single { it.name == "nested" } as LabNode.Container
        assertNotNull(root.preview)
        assertNotNull(root.summary)
        // LabScreen reads both only under `style == Full`, and JsonSubObjectViewHolder.kt:30-41
    ...
    fun `the container style is decided by depth, and an array is always Full`() {
        val nodes = LabTreeBuilder.build(sample())
        // LabJsonAdapter.kt:98-100 - `if (item.level == 1)` picked the preview/summary rendering.
    ...
    fun `display text is the quoted JSON rendering`() {
        val leaves = LabTreeBuilder.build(sample()).filterIsInstance<LabNode.Leaf>().associateBy { it.name }
        // JsonElementViewHolder.kt:51 used jsonElement.toString(), which quotes strings. The dialog
